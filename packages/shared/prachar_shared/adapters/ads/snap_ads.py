...
class SnapAdsAdapter(AdNetworkAdapter):
    ...
    def create_campaign(self, tokens: TokenSet, campaign: dict[str, Any]) -> str:
        ...
        campaign_id = f"snap-{digest}"
        logger.info("snap_ads.create_campaign stub -> %s", campaign_id)
        return campaign_id
    ...
    def upload_creative(self, tokens: TokenSet, creative: CreativeAsset) -> str:
        ...
        digest = hashlib.sha256(raw.encode("utf-8")).hexdigest()[:12]
        creative_id = f"snap-creative-{digest}"
        logger.info("snap_ads.upload_creative stub -> %s", creative_id)
        return creative_id
    ...
    def set_budget_bid(
        ...
        logger.info(
            "snap_ads.set_budget_bid stub campaign=%s budget=%s bid=%s idem=%s",
            campaign_id, budget, bid, idem,
        )

    def pause(self, tokens: TokenSet, campaign_id: str) -> None:
        idem = f"{campaign_id}:pause"
        logger.info("snap_ads.pause stub campaign=%s idem=%s", campaign_id, idem)
    ...
    def stats(self, tokens: TokenSet, campaign_id: str, since: datetime) -> list[MetricEvent]:
        ...
        for d in range(3):
            ...
            for metric, base_val in (
                ("impressions", 500.0), ("swipes", 15.0), ("spend", 5.0), ("conversions", 0.8),
            ):
                events.append(
                    MetricEvent(
                        ...
                        metric=metric,
                        value=round(base_val + (seed % 7) * (d + 1) * 0.1, 4),
                        ts=ts,
                    )
                )
        return events
